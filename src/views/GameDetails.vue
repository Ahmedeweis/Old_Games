<template>
  ...
  <div class="z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
    <div class="flex flex-col md:flex-row justify-between items-start w-full gap-6">
      ...
      <div class="w-full md:w-3/4 order-1 md:order-2">
        <div class="w-full flex flex-col items-end bg-[#F8F8F4] rounded-xl shadow-lg overflow-hidden">
          ...
          <div class="flex flex-col sm:flex-row justify-between items-center w-full p-4 md:p-6 gap-4">
             <button class="bg-[#009089] text-white px-6 py-2 rounded-lg hover:bg-[#007a74] transition duration-300 w-full sm:w-auto order-2 sm:order-1">
              مزيد عن اللعبة
            </button>
            <div class="flex justify-end items-center gap-3 overflow-x-auto w-full sm:w-auto order-1 sm:order-2 pb-2 sm:pb-0">
              <img v-for="i in 4" :key="i" :src="game?.cover_image" alt="Thumbnail" class="rounded-lg shadow-md w-12 h-12 object-cover flex-shrink-0"
                @error="e => e.target.src = '/games/default.jpg'">
            </div>
          </div>
        </div>
      ...
      <div class="w-full md:w-1/4 bg-[#F8F8F4] p-6 rounded-xl shadow-lg flex flex-col justify-between text-right gap-6 order-2 md:order-1">
          ...
          <a v-if="game?.telegram_link" :href="game.telegram_link" target="_blank"
            ...
            <svg class="w-6 h-6" viewBox="0 0 240 240" xmlns="http://www.w3.org/2000/svg">
              <g fill="none" fill-rule="evenodd">
                ...
                <path fill="#C8DAEA" d="M100.632 161.98l-3.958 45.09c5.65 0 8.12-2.44 11.21-5.36l26.86-25.58 55.78 40.67c10.23 5.63 17.63 2.67 20.26-9.49l36.69-172.55.002-.002c2.25-10.61-3.86-14.63-13.18-11.64l-210.12 81.03c-14.33 5.7-14.26 13.81-2.61 17.49l53.83 16.81 124.73-78.42c5.87-3.74 11.23-1.75 6.82 2.4L100.63 161.98z" />
              </g>
            </svg>
          </a>
          ...
          <div class="flex items-center justify-between text-gray-500">
            <div class="text-right">
              ...
              <div class="flex mt-1 justify-end gap-0.5">
                <img v-for="i in 5" :key="i" src="../assets/imgs/star.png" class="w-4" :class="{'opacity-30': i > Math.round(game?.rating || 0)}" alt="star">
              </div>
            </div>
            <div class="text-right">
      ...
      <div class="w-full md:w-3/4 order-3 md:order-2">
        <div class="bg-[#F8F8F4] rounded-xl shadow-lg p-4 md:p-6 w-full text-right">
          ...
          <div class="space-y-6">
            ...
            <div class="bg-white p-4 md:p-6 rounded-lg shadow-sm">
              ...
              <p class="text-gray-700 leading-loose">لا يتوفر أسلوب لعب مفصل حالياً، اللعبة تعتمد على أسلوب {{ game?.genre_name_ar }} التقليدي.</p>
            </div>
          </div>
        </div>
      </div>
